  ...
  useEffect(() => {
    const fetchRewardsAndUser = async () => {
      ...
      try {
        ...
        const userPointsMap = fetchedRewards.reduce((acc, reward) => {
          if (!acc[reward.userId]) {
            acc[reward.userId] = { ...reward };
          } else {
            acc[reward.userId].points += reward.points; // Aggregate points
          }
          return acc;
        }, {} as Record<number, Reward>);

        const uniqueRewards = Object.values(userPointsMap).sort(
