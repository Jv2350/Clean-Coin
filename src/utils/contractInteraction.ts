...
const contractAddress = '0x2a0E1984023f515f29a5456E71aFAF2E956a1648';
...
export const claimReward = async (amount: string) => {
  try {
    ...
    // }
    if (balance.lt(amountBN)) {
      console.error(`Insufficient balance. Required: ${ethers.utils.formatUnits(amountBN, 18)} RWT, Available: ${ethers.utils.formatUnits(balance, 18)} RWT`);
      throw new Error(`Insufficient balance. Required: ${ethers.utils.formatUnits(amountBN, 18)} RWT, Available: ${ethers.utils.formatUnits(balance, 18)} RWT`);
  }
  
    // const gasEstimate = await contract.estimateGas.claimReward(amountBN);
    const gasEstimate = await contract.estimateGas.claimReward(amountBN).catch((error) => {
      console.error('Gas estimate failed:', error);
      return ethers.BigNumber.from(21000); // Default to a reasonable value
  });
